...
/**
 * Map key identifying one comp: its set plus its signature.
 *
 * NUL separates them because it cannot occur inside a signature, so it can
 * never collide with the data it delimits. Written as `\0` rather than a raw
 * 0x00 byte — that byte made grep classify this whole file as binary and skip
 * it in every recursive search, which is how two env vars read on lines 55 and
 * 58 were once concluded to be dead.
 *
 * Defined ONCE on purpose. The scan builds this key and the upsert reads it
 * back; if the two ever spelled the separator differently the lookup would
 * miss, silently re-minting comps that already exist rather than failing.
 */
function compKey(setNumber: number, signature: string): string {
  return `${setNumber}\0${signature}`;
}
...
async function scanBoards(
  ...
  for (;;) {
    ...
    for (const p of participants) {
      ...
      const key = compKey(p.set_number, identity.signature);
...
async function upsertComps(client: PoolClient, seeds: CompSeed[]): Promise<number[]> {
  ...
  for (let start = 0; start < seeds.length; start += WRITE_CHUNK) {
    ...
    for (const row of res.rows) idByKey.set(compKey(row.set_number, row.signature), row.id);
  }

  for (let i = 0; i < seeds.length; i++) {
    const id = idByKey.get(compKey(seeds[i].setNumber, seeds[i].signature));
    if (id === undefined) {
