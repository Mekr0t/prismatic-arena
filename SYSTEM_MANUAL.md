...
| `lib/emblems.test.ts` | the emblem → trait rule. The trait is the name minus " Emblem"; multi-word names survive ("Flora Fatalis"); non-emblems and a bare "Emblem" yield nothing. The grant line matches the wording every other set uses, and — the case the rule exists for — follows the NAME not the id, so "Ravager Emblem" (`DA_18_EmblemSlayer`) grants Ravager. An emblem for a trait the loading set does not have is refused, which is what stops another set's emblem inventing a grant |
| `lib/keywords.test.ts` | the glossary, whose risk is a confidently WRONG definition rather than a missing one. Reference resolution and its case-insensitivity; that Chill is the Attack Speed definition and not the healing one (the contradiction in Riot's own data that makes this table curated); Wound and Chill staying distinct; non-keyword `{{…}}` refs and bare names resolving to null so they keep being dropped; an unknown keyword returning null so the loader can report it; and every definition being non-empty, not restating its own name (the renderer already prefixes it), and keyed by the name it resolves to |
| `lib/stat-icons.test.ts` | the atlas vocabulary, which fails QUIETLY — a wrong rect shows a real glyph, just the wrong one, and a missing alias shows bare text beside stats that have icons. Every rect inside the atlas bounds; native sizes limited to the three the atlas uses (a wrong `n` renders a crop of the neighbouring glyph); no two stats sharing a rect; every icon carrying a spoken label. All three vocabularies converging on one key (`scaleap` / `AP` / `Ability Power`); case- and space-insensitive lookup; every alias resolving to a defined icon; an unknown stat returning null rather than a wrong icon; and every icon's own label resolving BACK to it, which caught `crit` labelled "Critical Strike" with no alias to match |
...
### `emblems.ts`
Reading the trait out of a trait emblem. `traitNameFromEmblem(itemName)` strips a trailing " Emblem" to give the trait's DISPLAY name; `emblemGrantDescription(itemName, liveTraitNames)` turns that into "The holder gains the X trait.", or null when the name matches no trait in the set being loaded (so it can never invent a grant for another set's emblem — the `items` table holds CDragon's whole global catalog under the live set number). **The rule keys on the display NAME, not the id, and set 18 is why:** `DA_18_EmblemSlayer` is named "Ravager Emblem" and grants Ravager, so an id-based rule gets that one wrong; CDragon's `associatedTraits` is empty on all 21 of the set's emblems. Set 18 publishes NOTHING for its emblems — `desc: null`, `effects: {}`, no `associatedTraits` — which is why the Library showed a named icon above an empty body. The loader fills the grant line only when CDragon has no text, so real descriptions take over automatically when Riot publishes them, with nothing to remove. The per-emblem BONUS line (Invoker's "On cast, gain Ability Power equal to 10% of Mana spent") is deliberately NOT synthesised: it is not published anywhere for set 18, and a guessed effect shown as fact is worse than a short entry. Shared by `planner-data.ts` (emblem → trait for the trait engine) and the loader. `EMBLEM_BONUSES` holds hand-transcribed per-emblem effects keyed by lowercase trait name, appended after the grant line with a blank line between; only about half the set's emblems have one, and an absent key is a real answer (grant only), not a gap. TRANSCRIBED, NEVER GUESSED — an entry is shown to users as fact. Temporary in the same way the set-18 trait bridge was: published text always wins, so these stop being reached the day Riot publishes emblem descriptions. Covered by `emblems.test.ts`.

### `keywords.ts`
Definitions for the TFT keywords CDragon references but does not inline. Most item text explains its own: Morellonomicon ships a rules block spelling out Burn and Wound. Others carry a TEMPLATE REFERENCE — `{{TFT_Keyword_Precision}}` — pointing at text that lives in the game client, not the published data, and the loader used to strip `{{…}}` wholesale, which is why Jeweled Gauntlet and Infinity Edge read "Gain Precision." and stopped. `keywordFor(ref)` resolves `TFT_Keyword_<Name>` (case-insensitive) to `{name, text}`; the loader substitutes a `«rules:«bold:Name»: text»` block, the SAME shape CDragon uses inline, so both paths render identically. **Curated, not harvested, and the distinction is load-bearing:** scraping the definitions out of entries that do spell them out was tried and is unsafe, because Riot's data contradicts itself — `TFT15_HiddenTech_AffinityForCold` ships "Chill: Reduce healing received by 20%" (Wound's effect) while `TFT4_Item_OrnnEternalWinter` has the correct "Chill: reduce Attack Speed", and a harvester takes whichever it reaches first. Each entry records the source that justifies it. Unresolved `TFT_Keyword_*` references are collected and reported ONCE at the end of `data:load`, so a new keyword surfaces instead of leaving a sentence citing something never explained. Covered by `keywords.test.ts`.

### `stat-icons.ts`
**The one place a stat is tied to its in-game glyph.** TFT's stat icons live in a single texture atlas (`ASSETS/UX/Fonts/TextIcons.tex`, served by CDragon as `texticons.png`, 256 x 512). `STAT_ICONS: Record<StatIconKey, StatIcon>` holds 14 rects as `{x, y, n, label}` — `n` is the NATIVE size, and the icons are not uniform (18, 20, 24 px), which is why the renderer scales each one by `size / n` rather than using a fixed cell. The rects are not guesses: ten are the `mTextureUV` values the game's own unit-info card declares in `clientstates/gameplay/ux/tft/tftunitinfo/uibase.cdtb.bin.json`; the other four (health, health regen, mana, mana regen) are absent from that file and were read off the atlas directly. `statIconKey(name)` maps THREE disagreeing vocabularies onto one key — CDragon's `%i:scaleAP%` token names, the unit grid's terse labels (`AP`, `MR`), and item stats' spelled-out ones (`Ability Power`, `Magic Resist`) — which is what stops the same stat showing an icon in one place and bare text in another. Also exports `ATLAS_W` / `ATLAS_H` / `ATLAS_URL`. Covered by `stat-icons.test.ts`.

### `components/StatIcon.tsx`
`StatIcon({ iconKey, size?, className? })` renders one glyph; `StatLabel({ label })` renders a stat's name preceded by its glyph when one exists (the label ALWAYS stays — the icon is a scanning aid, and a stat with no glyph must still read normally). The atlas arithmetic lives in the component rather than in CSS deliberately: a stylesheet copy of the same rects would eventually disagree with `stat-icons.ts` and show the wrong glyph, which nothing would catch. `rich.css` holds only `.si` layout. Used by `rich-text.tsx`, `UnitStatsGrid.tsx`, and both item stat lists (`game-data.tsx`, `Library.tsx`).

### `rich-text.tsx`
`'use client'` — renders the `«class:text»` token format that `load-static-data.ts` emits (CDragon semantic tags → colours). **`«icon:key»` is the one token whose content is a lookup key rather than text**: it draws the real game glyph via `StatIcon`, and replaced the old `«scale:AP»` text labels, which existed only because the icon could not be drawn. Both forms still parse — descriptions are resolved at LOAD time, so every row keeps whichever token it was written with until the next `data:load`. An unknown icon key falls through to a plain span so a new stat shows its name instead of vanishing. `richToPlain` substitutes an icon's `label`, otherwise a flattened tooltip would silently lose the stat. `RichText` PARSES the nesting-capable grammar into React spans (never innerHTML — no injection surface); `richToPlain` / `richFirstLine` flatten tokens for tooltips. Class names map 1:1 to `.rt-<class>` in `rich.css`. Plain (token-free) text renders unchanged.
...
### `library-data.ts`
`getLibraryData()` → `LibraryData` (units/traits/items/augments) for the library + popups. `LibItem.recipe: RecipePart[]` resolves the two components an item is built from — CDragon has always shipped `composition`, but the Library only ever used it to CLASSIFY an item as craftable and never showed the recipe itself. Resolved against a lookup built from the RAW rows BEFORE the name dedupe, because a component's own row can lose that dedupe to a same-named one (set 18 ships `DA_Component_BFSword` beside the classic `TFT_Item_BFSword`). All-or-nothing: half a recipe is more confusing than none. Rendered as icons alone (`ItemRecipe` in `game-data.tsx`), with the component name kept in `alt`/`title` so it stays the accessible name and is still available on hover; a component with no icon falls back to its name rather than an empty slot. 292 set-18 items have one; 16 of 20 emblems do. Item rows are gated by `set-config.isSetItem(set, id)` — the cross-set `TFT_Item_` pool plus the set's own namespace — NOT a hardcoded `TFT{n}_Item_` prefix.  Descriptions carry `«class:…»` rich tokens (from `load-static-data.ts`); `LibItem.stats: ItemStat[]` holds item stat bonuses; trait `Breakpoint.effect` holds each breakpoint's resolved effect text.
