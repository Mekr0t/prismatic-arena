...
### Testing & CI
`npm test` is `tsx --test "src/**/*.test.ts"` — a GLOB since 2026-08-22, so a new `*.test.ts` anywhere under `src/` is picked up instead of being silently skipped (it used to name two files explicitly). **126 tests:**
