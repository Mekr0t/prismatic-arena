...
scripts/
  ...
  _persist-check.ts      persistMatch integration check against a SYNTHETIC match —
                         ...
                         and multi item arrays, and idempotency on a second call.
...
| `resolvePatchId(client, setNumber, gameVersion)` | Upserts `patches`, returns `id`. Also auto-advances `is_current` when the seen patch is numerically newer than the currently-flagged one (match data, not config, defines "current"). Runs on a transaction-scoped `PoolClient`. **LIVE-SET GATE:** patch strings are only comparable WITHIN a set — rotating game modes replay old sets on today's client version, so `"16.14"` exists for sets 1, 16 and 17 in real data. Only the **live set** (`MAX(set_number)` from `units`, memoized 5 min) may touch the flag, and the "what's current" read is scoped to that set, which self-heals an already-stolen flag. Without this, the first revival match on a new patch moved the flag to a set with no catalog and `currentSet()` blanked the Library/planner/comp names |

### `match-persist.ts`
`persistMatch(match, bucket = 'unknown')` — idempotent match persistence (early-exit on existing `match_id`, all sub-inserts `ON CONFLICT DO NOTHING`, carry heuristic, `patch_id` via `resolvePatchId`). Shared by the profile path **and** the match-fetch worker. Extracted from `profile-service.ts`.
